...
#include <inttypes.h>
#include <algorithm>
#include <vector> //discarded this idea, used traditional array instead
...
private:
    ...
    BPredPolicy policy;
    uint64_t PHT[4096]; //2^12 index values from GHR XOR lower_12 from PC possible
    uint64_t PHT_index; //2 bit index = taken(increment), none taken(decrement)
    uint64_t GHR; //by definition 12 bit long
    //declaring these in public, gave segmentation fault
